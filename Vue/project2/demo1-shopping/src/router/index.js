...
router.beforeEach((to, from, next) => {
  ...
  if(!authUrl.includes(to.path)) //to.path比to.name多了/
  {
   next();//放行
  }
  //如果要访问需要登录的路径
   ...
   console.log("认证")
   if(store.state.user.userInfo && store.state.user.userInfo.token )//store和this.$store是等价的。但是这里的this并不是如同组件一样，指向vue实例。所以通过导入store。
   {
     next()
   }
   else{
